        ...
        <?php foreach ($favPosts as $post): ?>
            <div class="fav-card">
                ...
                <div class="fav-card-body">
                    <a class="fav-card-title" href="/post/<?= $post->alias ?>">
                        <?= $post->title ?>
                    </a>
                    ...
                    <div class="fav-card-actions">
                        ...
                        <a class="btn" href="/post/<?= ($post->alias) ?>" title="Read More">
    ...
    <nav aria-label="Favorites pagination" class="mt-4">
      <ul class="pagination justify-content-center amazon-pagination">
        <li class="page-item <?= $currentPage <= 1 ? 'disabled' : '' ?>">
          <a class="page-link" href="/posts/favourites/<?= max($currentPage - 1, 1) ?>" tabindex="-1">
            <i class="bi bi-chevron-left"></i>
          </a>
        </li>
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
          <li class="page-item <?= $i == $currentPage ? 'active' : '' ?>">
            <a class="page-link" href="/posts/favourites/<?= $i ?>"><?= $i ?></a>
          </li>
        <?php endfor; ?>
        <li class="page-item <?= $currentPage >= $totalPages ? 'disabled' : '' ?>">
          <a class="page-link" href="/posts/favourites/<?= min($currentPage + 1, $totalPages) ?>">
